...
namespace Drupal\cron_weeklyfetch\Form;
 ...
 */
class CronWeeklyFetchForm extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    $config = $this->configFactory->get('cron_weeklyfetch.settings');
    ...
    $form['status']['intro'] = [
      '#type' => 'item',
      '#markup' => $this->t('The cron weekly fetch demonstrates hook_cron() and hook_queue_info() processing. If you have administrative privileges you can run cron from this page and see the results.'),
    ];

    $next_execution = $this->state->get('cron_weeklyfetch.next_execution');
    $next_execution = !empty($next_execution) ? $next_execution : REQUEST_TIME;

    $args = [
      '%time' => date_iso8601($this->state->get('cron_weeklyfetch.next_execution')),
      '%seconds' => $next_execution - REQUEST_TIME,
    ];
    $form['status']['last'] = [
      '#type' => 'item',
      '#markup' => $this->t('cron_weeklyfetch_cron() will next execute the first time cron runs after %time (%seconds seconds from now)', $args),
    ];
    ...
    if ($this->currentUser->hasPermission('administer site configuration')) {
      ...
      $form['cron_run']['cron_reset'] = [
        '#type' => 'checkbox',
        '#title' => $this->t("Run cron_weeklyfetch's cron regardless of whether interval has expired."),
        '#default_value' => FALSE,
      ];
    ...
    $queue_1 = $this->queue->get('cron_weeklyfetch_queue_1');
    $queue_2 = $this->queue->get('cron_weeklyfetch_queue_2');
    ...
    $form['cron_queue_setup']['queue'] = [
      ...
      '#options' => [
        'cron_weeklyfetch_queue_1' => $this->t('Queue 1'),
        'cron_weeklyfetch_queue_2' => $this->t('Queue 2'),
      ],
      '#default_value' => 'cron_weeklyfetch_queue_1',
    ];
    ...
    $form['configuration'] = [
      '#type' => 'details',
      '#title' => $this->t('Configuration of cron_weeklyfetch_cron()'),
      '#open' => TRUE,
    ];
    $form['configuration']['cron_weeklyfetch_interval'] = [
      '#type' => 'select',
      '#title' => $this->t('Cron interval'),
      '#description' => $this->t('Time after which cron_weeklyfetch_cron will respond to a processing request.'),
  ...
  public function cronRun(array &$form, FormStateInterface &$form_state) {
    $config = $this->configFactory->getEditable('cron_weeklyfetch.settings');

    $cron_reset = $form_state->getValue('cron_reset');
    if (!empty($cron_reset)) {
      $this->state->set('cron_weeklyfetch.next_execution', 0);
    }

    // Use a state variable to signal that cron was run manually from this form.
    $this->state->set('cron_weeklyfetch_show_status_message', TRUE);
    if ($this->cron->run()) {
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->configFactory->getEditable('cron_weeklyfetch.settings')
      ->set('interval', $form_state->getValue('cron_weeklyfetch_interval'))
      ->save();
  ...
  protected function getEditableConfigNames() {
    return ['cron_weeklyfetch.settings'];
  }
